...
fn test_consistency() {
    ...
    let mut life_hash256 = ConwayFieldHash256::blank(N, N);
    ...
    life_hash256.randomize(Some(SEED), FILL_RATE);
...
fn test_get_single_and_multiple() {
    ...
    life_naive.randomize(Some(SEED), FILL_RATE);
    life_simd1.randomize(Some(SEED), FILL_RATE);    life_simd2.randomize(Some(SEED), FILL_RATE);
    life_hash.randomize(Some(SEED), FILL_RATE);
    life_hash256.randomize(Some(SEED), FILL_RATE);
    // life_shader.randomize(Some(SEED), FILL_RATE);
    ...
    let cells_hash256 = life_hash256.get_cells(.., ..);
    ...
    let mut iter_hash256 = cells_hash256.iter();
...
fn test_set_single_and_multiple() {
    ...
    let mut life_hash256_single = ConwayFieldHash256::blank(N, N);
    ...
    let mut life_hash256_multi = ConwayFieldHash256::blank(N, N);
    ...
    life_hash256_single.randomize(Some(SEED), FILL_RATE);
    ...
    life_hash256_multi.randomize(Some(SEED), FILL_RATE);
    ...
    life_hash256_multi.set_cells(.., .., states.iter().map(|&x| x));
    ...
    for y in 0..N {
        for x in 0..N {
            ...
            life_hash256_single.set_cell(x, y, states[x + y * N]);
